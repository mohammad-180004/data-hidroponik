...
// Jika pengecekan berhasil, maka gambar diunggah dan disimpan di folder dan database
	...
	if (move_uploaded_file($_FILES["imageFile"]["tmp_name"], $target_file)) {
		...
		$picture_name = date('Ymd_Hi-', $datum) . basename($_FILES["imageFile"]["name"]);
		$date_time = date("Y-m-d H:i:s");
		
		// Melakukan query INSERT untuk nama file ke tabel uploaded_pictures
		$dbc = new PDO("mysql:host=localhost;dbname=data_hidroponik_db","data-hidroponik", "+(JzZ55xedU-5G3k3R");
		$query = $dbc->prepare("INSERT INTO uploaded_pictures (PictureID, DateTime, PictureName) VALUES (NULL, :DateTime, :PictureName");
		$query->bindValue(":DateTime", $date_time);
		$query->bindValue(":PictureName", $picture_name);
		$query->execute();
